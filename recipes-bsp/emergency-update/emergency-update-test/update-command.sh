...
if test $ldver < 4
	...
	nand erase uldr
	nand write uldr 
	set mtdparts 'mtdparts=nx_2017:1024k@0k(uldr),1792k@1024k(u-boot),20m@3m(configure),6m@23m(kernel),350m@29m(root),45m@381m(update),-(extra)'
	set nandparts 'mtdparts=nx_2017:1024k@0k(uldr),1792k@@1024k(u-boot),20m@3m(configure),6m@23m(kernel),350m@29m(root),45m@381m(update),-(extra)'
	...
	display_strings "press i button to start update otherwise"
	display_strings "system will boot as normal after 10 seconds."
	if check_update
	then cleare_display_strings
